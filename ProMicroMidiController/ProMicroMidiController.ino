...
void loop() {
 ...
 //     digitalWrite(MOTOR_R, true);
  ...
  sum += midi_volume;
  float average = sum / HISTORY_BUFF_SIZE;

  if (abs(average - LAST_FADER_READING_SENT) >= 1) {
    ...
    midiEventPacket_t event = {0x0B, 0xB0 | MIDI_OUT_CH, VOLUME_CC_NUM, midi_volume};
    MidiUSB.sendMIDI(event);

    MidiUSB.flush();

    LAST_FADER_READING_SENT = midi_volume;
  } else {
